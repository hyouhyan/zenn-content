...
topics: ["GAS", "Discord", "天気予報", "Webhook", "気象庁"]
published: true
---
今度、2泊3日で広島旅行に行くんすよ。  
...
# 結論(ソースコード)
[Hyouhyan「[noticeWeatherWebhook](https://github.com/hyouhyan/noticeWeatherWebhook)」| GitHub
...
# 解説
## 気象庁API
気象庁APIは、気象庁が提供している天気予報のAPIです。  
ただ、提供していると言っても、気象庁が内部で使用しているものになるので、サポートの対象外です。
(まあ、使えるし、いいんじゃないかなーと思っています)

気象庁のURLと地域コード(広島なら340000)を指定することで、その地域の天気予報が取得できます。
例: 広島の天気予報
`https://www.jma.go.jp/bosai/forecast/data/forecast/340000.json`
ちなみに、地域コードは[ここ](https://www.jma.go.jp/bosai/common/const/area.json)で確認できます。
`https://www.jma.go.jp/bosai/common/const/area.json`

## Discord Webhook
Discord Webhookは、Discordに投稿するためのURLです。
認証とかを考えずPOSTするだけで使えるので、非常に使い勝手がいいです。

特筆することとすればEmbedぐらいでしょうか。
こちらを参考にしました。
https://qiita.com/hisuie08/items/5b63924156080694fc81

Embedを指定する時、`embeds`となっていることから分かるように、Embedは複数指定できます。
そのため、var embeds = []として、配列にPushしています。

## GAS
Google Apps Scriptは、Googleが提供しているスクリプトエンジンです。
本来ならLinuxでcronを使って定期実行的なことをしなければいけませんが、これがあれば、Linuxサーバーを用意せずとも、無料で定期実行ができます。

こんな感じで、時間ベースの1時間おきに設定しています。
[![Image from Gyazo](https://i.gyazo.com/16af277c226e80a4710a2debe8cf0301.png)](https://gyazo.com/16af277c226e80a4710a2debe8cf0301)

[![Image from Gyazo](https://i.gyazo.com/e0ae94e6a6bc60a47bd31d718614e550.png)](https://gyazo.com/e0ae94e6a6bc60a47bd31d718614e550)
...
# 参考文献
https://scrapbox.io/ke456memo/Discord_Webhookでお天気＆タスクbot
